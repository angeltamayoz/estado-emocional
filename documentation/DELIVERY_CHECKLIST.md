...
- [~] Scripts en Python que simulen registro de usuarios y carga de encuestas: `app/` (parcial — revisar `app/api.py`, `app/models.py`) ~
- [x] Manejo de archivos CSV en `data/` ✓
- [ ] Evidencia del uso de Git (commits, ramas, PRs) — poner capturas/links en `EVIDENCE/` ❗
...
- [~] Scripts en Python para limpieza y transformación: `app/analytics.py` (parcial — agregar notebooks o scripts de ejemplo) ~
- [~] Análisis exploratorio y visualizaciones (`app/analytics.py`): parcial — generar PNGs y ejemplos reproducibles ~
- [ ] Dashboard básico que muestre: estado emocional promedio por grupo, alertas, evolución temporal — Frontend/ endpoints partially cover this; verify and add screenshots.
- [ ] Evidencia visual (capturas del dashboard, gráficos) — add to `EVIDENCE/`.
